...
---
> **Nexus adaptation note (2026-04-14):** This skill was imported from the agentsys multi-tool framework. In the nexus context:
> - Ignore any `Task({ subagent_type: ... })` dispatch patterns — treat them as pseudocode for "run this as a sub-analysis." You (Claude Code with Opus 4.6) execute the logic directly.
> - Replace `$ARGUMENTS` references with the nexus pipeline's trigger-phrase matching. The user's request is already on your screen — there is no `$ARGUMENTS` variable.
> - Ignore any `AskUserQuestion(...)` pseudocode — use the real `AskUserQuestion` tool when applicable (see pipeline/01-interview.md for when to use it).
> - All other logic, templates, and patterns below are canonical. Use them.
