...
For each matched skill, `Read` its `path` field (resolved against `<repo-root>` from
SKILL.md §1). Embed the FULL content into the relevant Lead's prompt — do not
summarize, do not pass just the path.
