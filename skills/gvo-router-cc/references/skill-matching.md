...
2. Issue ONE opus call (Agent tool, model: "opus", subagent_type: "general-purpose"
   — specialized types like "code-reviewer" do not improve relevance scoring here)
...
**Cost:** 5 parallel `Read` calls (already done in Step 4) + 1 opus call. The opus
call's input size is dominated by the 5 SKILL.md bodies — typically 5K-25K input
tokens, small JSON output.
...
1. For each matched skill, `Read` `<repo-root>/skills/nexus/<skill.path>` (the registry
