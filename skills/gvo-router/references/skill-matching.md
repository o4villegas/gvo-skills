...
run `codebase_read_file` on its `path` field. Embed the **trimmed** content (per
"Loading Skills' Content" §) into the Step 4.5 input or directly into the relevant
Lead's prompt — do not summarize, do not pass just the path.
